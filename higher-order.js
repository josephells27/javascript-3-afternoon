...
//Code Here
let evenNumbers = mixedNumbers.filter(el =>  el %2 ===0 ? true : false);
...
//Code Here
let myStrongest = monstersInYourPocket.filter( el => el.CP >200 ? true : false);
...
let ordersTotal = orders.filter(el => el.price + el.tax); 
...
let bobsTotal = purchases.filter(el => el.owner == "Bob" ? true : false).reduce((total, el) =>  total + el.price, 0)
